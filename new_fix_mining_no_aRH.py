...
if (len(args) == 5) and (args[4] == 'GYPSY'):
	...
	d = {'RNase_H_like':'gRH', 'RNase_HI_RT_Bel':'gRT', 'Retrotrans_gag':'GAG', 'Retrotran_gag_2':'GAG', 'RNase_HI_RT_Ty1':'gRH', 'RT_pepA17':'gRT', 'RT_like':'gRT', 'RVT_2':'gRT', 'rve':'INT', 'RNase_HI_like':'aRH', 'RNase_HI_RT_Ty3':'gRH', 'RT_LTR':'gRT','pepsin_retropepsin_like':'PRo','retropepsin_like':'PRo'}
...
def sorting_elements(l,mass):
	...
	#print(aRH,len(aRH))
	if len(RT) == 0:
		#print('[ERROR] We`ve got loose an RT domain!')
		return
	elif len(RT) == 1:
		RT = RT[0] ### replacing aRH = [[c1,c2,name,fr]] to aRH = [c1,c2,name,fr]
	else:
		delta = 20000
		top_RT = []
		for i in RT:
	...
	for i in mass:
		if 'gRT' in i:
			...
			lenght = max(c,(-c))
			#print(c)
			if (lenght < 3500) and (lenght < top_len):
				#print(aRH,c, i,end='\t')
				#print('\n')
				top_len = lenght
				top_gRT = i
	...
	'''
	#print(aRH, top_len,top_gRT,end='\t')
	#print()
	#print(top_gRT)
	'''
	if top_RT != []:
		#a = min(aRH[0],top_gRT[0])
		#b = max(aRH[1],top_gRT[1])
		a = aRH[0]
		#''
		if a ==aRH[0]:
			left = aRH
		...
		left = aRH
		#print(left)
		#is_good += ((top_gRT[1]-top_gRT[0]+1)/3)
		#print('RT has ' + str((top_gRT[1]-top_gRT[0]+1)/3) + 'length!')
	else:
	...
	#print(a,b)
	#print(find_tether_domain(l[1],mass,a,-1),top_gRT,find_tether_domain(l[1],mass,a,1),find_tether_domain(l[1],mass,b,-1),aRH,find_tether_domain(l[1],mass,b,1),end='\n')
	#print()
	#print(mass)
	...
	PRo_edge = '*'
	p = [left[0]-1,'','default']
	last_domain = left[2] #- need to try
	domains_from_left = []
	while p != ['','','','']:
	...
	LTR_char = []
	if edge1 - edge0 > 1000: 
		with open('find_LTR_2.fa','w') as blastf:
			toWrite = '>' + str(l[0].id) + '_Left\n' + str(l[0].seq[0:edge0])
			blastf.write(toWrite)
		with open('find_LTR_1.fa','w') as blastf:
			toWrite = '>' + str(l[0].id) + '_Right\n' + str(l[0].seq[edge1:])
			blastf.write(toWrite)
		#if '-trf' in args:
		...
		#else:
		call('makeblastdb -in find_LTR_1.fa -dbtype nucl -out findLTRs_DB ', shell = 'False', stdout=open(os.devnull, "w"), stderr=subprocess.STDOUT)
		call('blastn -query find_LTR_2.fa -db findLTRs_DB -out blastn_LTR.out -outfmt 6 -evalue 1e-3 -num_threads 4 ', shell = 'True') #-max_hsps 5
		...
		'''for i in r:
			for j in r:
				if i != j:
					...
					if float(i.split('\t')[11].split('\n')[0]) == float(j.split('\t')[11].split('\n')[0]) and (int(i.split('\t')[6]) <int(i.split('\t')[7])) and (int(i.split('\t')[8])< int(i.split('\t')[9])) and (int(j.split('\t')[3]) > 100) and (int(j.split('\t')[3]) < 1500):
						mi = min(int(j.split('\t')[6]),int(j.split('\t')[8]))
						ma = max(int(j.split('\t')[7]),int(j.split('\t')[9]))
						if (mi < edge0) and (ma > edge1) and (edge0 - mi < delta_l) and (ma - edge1 < delta_r):
							delta_r = ma - edge1
							delta_l = edge0 - mi
							new_edge0 = mi
							new_edge1 = ma
		...
		for i in r:
			if (int(i.split('\t')[6]) < int(i.split('\t')[7])) and (int(i.split('\t')[8]) < int(i.split('\t')[9])) and (100 < int(i.split('\t')[3]) < (edge1-edge0)):
				mi = int(i.split('\t')[6])
				ma = int(i.split('\t')[9])
				if (edge0 - mi < delta_l) and (ma < delta_r):
					delta_r = ma
					delta_l = edge0 - mi
					new_edge0 = mi
					new_edge1 = ma
					LTR_char = [float(i.split('\t')[2]),int(i.split('\t')[3])] # %identity, length of LTRs
	...
	p = '>' + str(l[0].id) +  '_domain-structure:' + end_structure + LTR_char + '|Score=' + str(is_good) + '\n' + str(l[0].seq)[edge0:edge1]+ '\n'
	a = []
	a.append((edge1-edge0))
	a.append(edge0)
	a.append(edge1)
	a.append(end_structure)
