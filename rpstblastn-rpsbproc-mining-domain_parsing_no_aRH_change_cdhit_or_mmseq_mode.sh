...
if [ "$4" = "GYPSY" ]; then
	...
	python3 $DARTS/new_1step_rpsbalstn-rpsbproc_parser_no_aRH.py $1 $2.step2g.rpsbproc.result $2
else
	...
	python3 $DARTS/new_1step_rpsbalstn-rpsbproc_parser_no_aRH.py $1 $2.step2.rpsbproc.result $2
fi
...
if [ "$4" = "GYPSY" ]; then
	...
	python3 $DARTS/new_fix_mining_no_aRH.py RT_and_approximates_$2_genome.fa $2.step3.rpsbproc.result $2 $4
else
	...
	python3 $DARTS/new_fix_mining_no_aRH.py RT_and_approximates_$2_genome.fa $2.stepX4.rpsbproc.result $2
fi
...
python3 $DARTS/parse_prot_seq_domains_no_aRH.py prot_domains.fa $2_elements 

python3 $DARTS/replacing_star_to_X.py $2 'gRT' 
python3 $DARTS/replacing_star_to_X.py $2 'gRH' 
python3 $DARTS/replacing_star_to_X.py $2 'aRH' 
python3 $DARTS/replacing_star_to_X.py $2 'GAG' 
python3 $DARTS/replacing_star_to_X.py $2 'INT' 
python3 $DARTS/replacing_star_to_X.py $2 'PRo' 

CD="cdhit"
if [ "$3" = "$CD" ]; then
	cdhit -i gRT_$2_elements.faa -c 0.8 -aL 0.8 -d 150 -o gRT_$2_elements.cdhit80.faa
	python3 $DARTS/delegate_selection.py $2 
	python3 $DARTS/del_cluster_amounts_from_name.py gRT_$2_elements.faa gRT_$2_elements_after_cdhit80_new_delegates.faa
	python3 $DARTS/find_another_elements_delegate.py $2 rewrited_gRT_$2_elements_after_cdhit80_new_delegates.faa
	...
	python3 $DARTS/mod_mmseq_outfile_farser.py gRT_$2_elements.mmseqs80-80_cluster.tsv gRT_$2_elements.faa
	python3 $DARTS/del_cluster_amounts_from_name.py gRT_$2_elements.faa mmseq_delegates_gRT_$2_elements.faa
	python3 $DARTS/find_another_elements_delegate.py $2 rewrited_mmseq_delegates_gRT_$2_elements.faa ### OR gRT_$2_elements_after_cdhit80_new_delegates.faa
	for i in {'gRH','INT','GAG','PRo'}; do python $DARTS/turn_back_cluster_amounts_to_name.py mmseq_delegates_gRT_$2_elements.faa $i-domain_$2_elements_delegates.faa; done
	mkdir mmseq2-80
